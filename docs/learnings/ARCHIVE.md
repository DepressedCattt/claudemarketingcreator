...
| **Hybrid Video Workflow** | 3203 | Background plate system: registry, VideoPlate component, scrim table, Kling prompt templates. |
| **Composition Guidelines: Spatial, Hierarchy, Contrast** | 3600 | Three rules from SaaS Showcase audit. Focal sizing (30–60%), no text-visual overlap, 4.5:1 contrast. Before/after tables. |
...
### CATEGORY B+ — Component Animation & Complex Ad Builds

| Section | Line | What it covers |
|---|---|---|
| **Component Animation (AE Extract)** | 3700 | Simple component animation subcategory. AE source data, spring configs, component types, simple vs complex definition. |
| **CalmlyAd: Chaos-to-Calm Narrative Ad** | 3800 | Most iterated ad. Kinetic typography, epicyclic blob motion, dynamic camera focus, layered camera effects, narrative drag interaction, SVG envelope, 4-beat tagline. |
| **§3810 What Worked** | 3810 | 9 techniques that shipped: typography cascade, envelope, screen shake, epicycles, coast phase, dynamic camera, layered effects, 4-beat tagline, narrative drag. |
| **§3820 What Didn't Work** | 3820 | 8 mistakes and fixes: continuous drift, dashboard breathing, scale overshoot, sine/cos drift, quadratic ease-in, instant spawn, wavy underline, text overlap. |
| **§3830 Spring Config Reference** | 3830 | Complete table of spring configs with stiffness/damping/mass and observed behavior. |
| **§3840 Timing & Pacing** | 3840 | Act duration ratios, timing lessons, stagger intervals, shake duration, camera effect coupling. |
| **§3850 Reusable Patterns** | 3850 | 10 extractable patterns for future ads. |
...
### CATEGORY G — Reference Ad Insights *(Read when recreating a competitor/reference ad style)*

> Compact, agent-readable summaries. Each row is immediately actionable.
> For deep beat-by-beat breakdowns see **Category H** (raw archive — skip unless you need full detail).

| Section | What it covers |
|---|---|
| **G section body** | One compact table per analysed ad: design system, motion feel, spring configs, easing, timing, key technique to steal |

---

### CATEGORY H — Reference Ad Full Analyses *(Raw archive — deep dive only, skip on first pass)*

| Section | What it covers |
|---|---|
| **H section body** | Full Gemini 2.5 Pro video analyses. Beat-by-beat animation sequences, detailed motion profiles, full Remotion recreation notes. |
...
## G — Reference Ad Insights

> Compact summaries of reference ads analysed via Gemini 2.5 Pro.
> **Read these when recreating a similar ad style.** Each entry gives you everything needed to match the feel in Remotion.
> For the full beat-by-beat breakdown of any entry see → **Section H** (raw archive at the bottom of this file).
>
> *New entries are auto-appended by `npm run analyze` or the Studio UI `🔬 Analyse Ad` button.*

---

*(No entries yet — run your first analysis to populate this section.)*

---

### G.1 — LangEase SaaS · videoplayback (1) (2026-03-20)

| | |
|---|---|
| **Design language** | Minimal clean-tech (Apple-like) · off-white `#FDFDFE` bg · blue `#3A61FF` accent · glassmorphism UI cards |
| **Motion feel** | Snappy and weightless · sharp ease-out deceleration · no bounce · critically damped · purely graphic/digital |
| **Spring config** | `{ stiffness: 200, damping: 25, mass: 0.8 }` — for UI card reveals and button states |
| **Easing** | `Easing.out(Easing.cubic)` for slides and fades · `Easing.inOut(Easing.quad)` for 3D tunnel |
| **Typography** | Blur-in + scale from 0.95→1.0 · word-by-word stagger ~4-5 frames (133-166ms) · one gradient headline |
| **Transitions** | Hard cuts synced to beat + phone-flies-into-camera wipe · 150-300ms · UI cards fly out and become next scene bg |
| **Scene timing** | ~60-75f per beat at 30fps · transitions 6-9f · text reveals ~15f total |
| **Hook technique** | Rapid-fire text replacements synced to vocal countdown in music — 4 concepts in 4 seconds |
| **Key technique to steal** | UI elements travel ACROSS the cut (phone at 00:10, cards at 00:18) — they create the transition, not just play in it |
| **Full analysis** | → H.1 |
...
### "On-Ice" Inertia Easing — Double-Pulse Smootherstep

When the user asks for movement that "speeds up, slows down, speeds up, slows down" — like a camera on ice, or an FPV drone with high inertia — use a **double-pulse smootherstep** applied to the global `t` before passing it to splines or interpolators.

**How it works:**
```ts
// Smootherstep: 6t⁵ − 15t⁴ + 10t³  (zero velocity at both endpoints)
const ss = (x: number) => x * x * x * (x * (x * 6 - 15) + 10);

// Apply twice: once over [0, 0.5], once over [0.5, 1]
const raw = clamp(frame / totalFrames, 0, 1);
const t   = raw < 0.5 ? ss(raw * 2) * 0.5 : 0.5 + ss((raw - 0.5) * 2) * 0.5;
```

**Velocity profile this produces:**
| Position in shot | Velocity | Feel |
|---|---|---|
| t = 0.00 | 0 | Camera at rest, building inertia |
| t = 0.25 | peak | Gliding at full speed |
| t = 0.50 | ~0 | Momentary float — frame lingers |
| t = 0.75 | peak | Second momentum burst |
| t = 1.00 | 0 | Settles into final position |

**Why this works:** The mid-point near-pause naturally lands at the narrative beat where Phase 1 ends and Phase 2 begins (rise peak / start of pull-back), so the easing curve *coincides* with story structure. The viewer gets a beat to absorb the screen content before the reveal.

**When to use:** Any shot where the user describes: "on ice", "with inertia", "speeds up and slows down", "like an FPV drone", "floaty", "has weight."

---

### Camera Angle References Are Recommendations, Not Fixed Placement

When the user describes a camera angle or position — e.g. "start worm's-eye looking up at the screen", "pull back to reveal the phone", "arc slightly to the right" — treat this as **directorial intent**, not an exact coordinate specification.

**What this means in practice:**
- Take the *feeling* the user is describing and translate it into the best-looking camera path.
- Do not lock numbers to the description if a slightly different value looks smoother or more cinematic.
- The description tells you the *narrative* ("start low and intimate, end wide and revealing"); you own the *execution* (exact positions, spline waypoints, lookAt targets, FOV values).
- Prioritise fluidity over literal accuracy. A path that matches the intent but moves beautifully is always preferred over one that hits the described coordinates but feels mechanical.

**Example:** "Start below the phone looking up, then pull back" → interpret as a pedestal-rise + dolly-out. The exact starting depth, height, and angle are yours to tune for the best shot composition — not constrained to specific values unless the user says so.
...
## Hybrid Video Workflow — Background Plates

*Added: hybrid Claude + Kling/Runway approach. Reduces hallucination risk by keeping Claude responsible for all text, UI, and animation, while video generation models supply atmospheric background depth.*

---

### The Problem This Solves

Video generation models (Kling, Runway Gen-3, Sora) cannot reliably render legible text, precise UI, or frame-perfect pacing. Claude/Remotion cannot easily generate photorealistic environmental depth. The hybrid approach gives each tool what it is best at.

- **Claude / Remotion:** text, UI components, 3D product models, animation timing, pacing, wipes, all foreground elements
- **Video gen model:** atmospheric background plates — salon interiors, abstract particles, studio environments — no text, no UI, fully loopable
...
```
1. Write a prompt from the templates in public/video/README.md
2. Generate a 5s loopable plate in Kling (or Runway Gen-3 / Sora)
3. Export as 1080×1920 H.264 MP4, no audio
4. Drop into public/video/  →  naming: [brand-slug]-[mood]-bg.mp4
5. Add one entry to src/data/videoPlates.ts  ←  this is the registration step
6. Claude reads videoPlates.ts automatically (Cursor rule fires on composition open)
7. Add VideoPlate to composition:  <VideoPlate plateId="haven-warm" scrimColor={C.bg} />
8. Tune opacity + scrimOpacity if needed (see scrim table below)
9. Render and review
```

---

### `VideoPlate` Component

**File:** `src/components/VideoPlate.tsx`
...
// First child of root AbsoluteFill, BEFORE camera motion wrapper:
<VideoPlate plateId="haven-warm" scrimColor={C.bg} />
```
Defaults for opacity and scrimOpacity come from the registry entry. Only `scrimColor` needs to be set — use your composition's `C.bg` token.
...
**Placement rule:** Always place `VideoPlate` as the first child of the composition's root `AbsoluteFill`, before any camera motion wrapper `div` and outside any `<Sequence>`. It must never be inside a `<Sequence>`.

---

### Scrim Formula Table

The scrim is a flat color div rendered on top of the video plate. It acts as a palette color grade — tinting the video to feel native to the composition's color system. Set `scrimColor` to the composition's `C.bg` token.

| Palette type | `scrimOpacity` range | Example |
|---|---|---|
| Light (cream, ivory, white) | 0.55–0.65 | HavenAd `#FAF7F3` |
| Mid-warm (warm grey, muted) | 0.48–0.55 | SharedSalon platform |
| Dark (navy, near-black, espresso) | 0.35–0.45 | ElumeAd `#0C0A0D` |

Start at the midpoint of the range and adjust until the background reads as environmental depth without competing with the foreground. If the background is too visible, increase `scrimOpacity`. If it disappears entirely, reduce it.

---

### Registry — `src/data/videoPlates.ts`

Every plate gets one entry. This is the single source of truth Claude reads before writing any composition. The Cursor rule (`.cursor/rules/video-plates.mdc`) automatically instructs Claude to check this file.

Entry structure:
```typescript
"haven-warm": {
  src:                 staticFile("video/haven-warm-bg.mp4"),
  mood:                "warm luxury salon interior, cream and terracotta bokeh",
  paletteHint:         "#FAF7F3",   // C.bg token to use as scrimColor
  defaultOpacity:      0.40,
...
Adding a new plate = adding one entry here. That is the entire registration step.

---

### Prompt Templates for Kling / Runway

See full templates in `public/video/README.md`. Short reference:

**Warm luxury salon** (HavenAd, LumenAd):
> Warm softly lit luxury salon interior. Cream and terracotta tones. Gentle bokeh from background candlelight. Slow imperceptible camera drift. Shallow depth of field. No people, no text, no UI. 9:16, 5s, seamlessly loopable.

**Dark editorial beauty** (ElumeAd):
> Cinematic dark interior, silk drapes, gold rim light from frame edge. Near-black, champagne and amber. Abstract slow motion. No people, no text. 9:16, 5s loopable.

**SaaS / tech** (FlowDesk, Meridian):
> Abstract dark blue-indigo digital environment. Soft luminous particles drifting slowly. Very deep bokeh. No UI, no text, no objects. Premium cinematic. 9:16, 5s loopable.

**Always append to every prompt:** `No people, no text, no UI, no logos.`

---

### Decision Rules

**Single looping plate** — use for mood-consistent ads where the background palette doesn't change significantly across scenes (HavenAd, ElumeAd, LumenAd).

**Per-scene plate swap via `<Sequence>`** — use for ads with major mood shifts (e.g. SharedSalonFinalAd goes from cream light to near-black dark CTA). Wrap each `VideoPlate` in its own `<Sequence>` at the correct frame offset.

**No plate** — skip for:
- Compositions using Three.js / `ThreeCanvas` as the full background (AwesomeAd, CinemaRevealAd) — 3D environment already provides depth
- Config-driven ads using `GradientBackground` — use the existing `videoSrc` prop on that component instead
...
> Full Gemini 2.5 Pro video analyses. Beat-by-beat animation sequences, detailed motion profiles, complete Remotion recreation notes.
> **Skip this section unless you need deep detail on a specific ad.** Use the compact summaries in → **Section G** instead.


---

### H.1 — videoplayback (1) (2026-03-20)

An analysis of the Zelios / LangEase SaaS marketing video ad.
...
-   **Color palette**:
    -   Primary: Blue `#3A61FF`
    -   Secondary: Dark Text `#1E1E1E`
    -   Accent: Gradient Blue-Purple (used in progress bar and "Turn" text) from `#879AF6` to `#C1A6FF`. Confetti also uses pinks and teals.
    -   Background: Off-white `#FDFDFE` / Pure White `#FFFFFF`.
-   **Typography**:
    -   Font Style: A modern, geometric sans-serif, similar to SF Pro or Inter.
    -   Dominant Weights: Medium for most text, Bold/Semibold for key headlines ("Turn **Books**").
    -   Size Hierarchy: Clear hierarchy with large hero text and smaller UI/caption text.
    -   Special Treatment: The word "Turn" (00:00) uses a subtle blue gradient.
-   **Background treatment**: Solid, clean off-white background throughout. UI elements use subtle drop shadows and glassmorphism (frosted glass effect with a soft border).
-   **Design language**: Minimal, premium, kinetic. It borrows heavily from a clean, modern tech aesthetic (similar to Apple's marketing).
-   **Text contrast strategy**: High contrast is maintained by placing dark or colored text directly onto the light, solid background. No complex text-over-image issues.

---

### MOTION PROFILE

-   **Kinetic energy**: Snappy and digitally precise. Motion is fast, responsive, and feels very light. There's a strong sense of immediacy.
-   **Dominant easing**: Sharp ease-out deceleration. Elements fly in at high speed and come to a near-instant, but smooth, stop. There is little to no follow-through or anticipation. The 3D phone tunnel (00:07) has a smoother, continuous ease-in-out feel.
-   **Motion blur**: Yes, subtle motion blur is present on fast-moving elements, particularly the text reveals, the 3D phone tunnel, and the UI cards that fly in. It implies high velocity and smooths out the rapid animations.
-   **Spring / bounce**: Almost no perceptible bounce or overshoot. The motion feels like a critically damped spring with very high stiffness. Estimate: stiffness around 200-220.
-   **Inertia feel**: Completely weightless. The motion is purely graphic and digital, not trying to simulate physical properties like mass or friction.
-   **Idle / ambient animations**:
    -   A subtle shimmering gradient sweep across the progress bar (00:11).
    -   A sparkling particle animation on the "Distribute" word and the final logo reveal (00:26).

---

### ANIMATION SEQUENCE

-   **00:00** — Text "Turn Books" — Blurs and scales in quickly — 0.5s
-   **00:01** — Text "Audio" — Blurs and scales in quickly, replacing previous text — 0.5s
-   **00:02** — Text "Any language" — Blurs and scales in — 0.5s
-   **00:03** — UI element (Folder icon) — Scales up with a slight overshoot — 0.7s
-   **00:04** — Text "Instantly" — Blurs and scales in — 0.5s
-   **00:05** — UI element (Glassmorphic window) — Wipes in from the center outwards — 0.5s
-   **00:06** — Text "Just drop and" — Blurs and scales in — 0.5s
-   **00:07** — 3D phone tunnel — Zooms in, phones scroll past camera in a parallax effect — 3.0s
-   **00:07** — Tunnel Text "Books." — Fades and scales in — 1.0s
-   **00:08** — Tunnel Text "Books. Audio. Video" — Fades and scales in — 1.0s
-   **00:09** — Tunnel Text "All In One Platform" — Fades and scales in — 1.0s
-   **00:10** — Transition — Last phone in tunnel flies toward camera, screen fills with a blue-to-white radial wipe — 0.5s
-   **00:11** — UI element (Progress bar) — Bar fills from left to right (81 to 99) with number counter — 2.0s
-   **00:13** — UI element ("Done" checkmark) — Circle scales in, checkmark draws itself, confetti explodes outwards — 1.0s
-   **00:14** — UI elements (Cards) — Four media cards slide and scale in from off-screen to form a grid — 1.5s
-   **00:16** — UI screen (Dashboard) — The card grid shrinks and integrates into a full browser UI that fades/scales in — 1.0s
-   **00:17** — Animated cursor — Clicks on the "History Lecture" card, which highlights — 1.0s
-   **00:18** — Transition — The card flies towards the camera, duplicates, and spins in 3D space — 1.0s
-   **00:19** — Text "Multiple Languages" — Fades and scales in over the spinning cards — 1.0s
-   **00:20** — UI screen (List view) — Transitions back to the browser UI, now in a list view — 1.0s
-   **00:21** — Animated cursor — Drags and selects "How-to Video" list item — 1.0s
-   **00:22** — UI element (Button) — "Distribute To Youtube" button appears — 0.5s
-   **00:23** — Animated cursor — Clicks the button, which depresses and changes color — 1.0s
-   **00:25** — Text "Translate." — A sparkle particle animates along the path of the letters, revealing them — 1.0s
-   **00:26** — Text "Translate. Dub. Distribute" — Revealed word-by-word with a fade/scale-up, ending with a sparkle — 2.0s
-   **00:28** — Logo "LangEase" — Fades and slides up slightly — 1.5s
-   **00:30** — URL "langease.ai" — Fades in below the logo — 1.0s

---

### TYPOGRAPHY ANIMATION

-   **Reveal method**: Primarily a fast blur-in combined with a slight scale-up (from ~0.95 to 1.0) for headlines. The final tagline "Translate. Dub. Distribute" uses a word-by-word fade-in and scale-up. "Translate." is revealed via a particle "write-on" effect.
-   **Consistency**: The blur-in/scale-up method is used consistently for most of the short, impactful headlines.
-   **Headline treatments**: One instance of gradient text ("Turn"). The final tagline is punctuated with a sparkle emoji/animation. Otherwise, the text is clean and solid-colored.
-   **Exit animations**: Text elements typically hard-cut or are wiped away by the next scene transition rather than animating out.
-   **Stagger timing**: For "Translate. Dub. Distribute", the stagger delay between each word is approximately **4-5 frames (133-166ms)**.

---

### TRANSITIONS & CUTS

-   **Scene transition style**: A dynamic mix including:
    -   **Hard cuts** synchronized to the music beat.
    -   **Element-driven wipes**: The phone at 00:10 flies into the camera, its screen acting as a transition wipe.
    -   **Zoom-in punch**: The transition into the 3D phone tunnel at 00:07.
    -   **3D element motion**: At 00:18, UI cards fly out from the interface and spin, forming the background for the next scene.
-   **Average transition duration**: Very fast, typically between 150ms and 300ms.
-   **Connective motion**: Yes. The phone at 00:10 and the UI cards at 00:18 are key elements that travel across the cut, creating a seamless and motivated transition between different abstract and UI-focused scenes.
...
-   **Number of distinct scenes / beats**: Approximately 15 major beats.
-   **Average beat length**: ~2.2 seconds. The pacing is very rapid, especially in the first 10 seconds.
-   **Audio sync**: Motion is tightly synchronized to the upbeat, four-on-the-floor electronic music track (~125 bpm). Key animations, cuts, and text reveals land precisely on the beat, creating a strong sense of rhythm.
-   **Hook (first 2 seconds)**: The video uses rapid-fire text ("Turn Books", "Audio") synced to a vocal countdown in the music ("one, two, three, four"), immediately establishing a fast pace and grabbing attention.
-   **CTA (final 3 seconds)**: The final CTA is the product logo and URL. It's presented cleanly and holds for a few seconds after a clear tagline ("Translate. Dub. Distribute"), allowing the brand name to sink in.

---

### PRODUCT / UI SHOWCASE

-   **Display method**: A combination of abstract UI elements (the folder icon, the floating cards) and a stylized, clean browser window mockup showing the product's dashboard.
-   **Feature highlight technique**: An animated cursor is used to guide the viewer's eye and demonstrate user actions like clicking on a video and pressing the "Distribute To Youtube" button.
-   **Screen content legibility**: Impressionistic. Key labels ("History Lecture", "Span travel") are readable, but the goal is to convey the product's function (a media library for content translation and distribution) rather than for the user to read every UI detail.

---

### REMOTION RECREATION NOTES

1.  **Spring config**: For the snappy UI animations (card reveals, button clicks), use a high-stiffness, high-damping spring to eliminate bounce.
    ```typescript
    // For snappy, non-bouncy UI motion
    const snappySpring = useSpring({ 
      frame, 
      fps, 
      config: { stiffness: 200, damping: 25, mass: 0.8 } 
    });
    ```
2.  **interpolate() easing**: For non-spring animations like fades and fast slides, use a sharp ease-out curve. A cubic or quintic ease-out will work well.
    ```typescript
    // For fast entrances that slow down abruptly
    const scale = interpolate(frame, [0, 15], [0.9, 1], { 
      easing: Easing.out(Easing.cubic) 
    });
    const opacity = interpolate(frame, [0, 10], [0, 1]);
    ```
3.  **Key timing values** (at 30fps):
    -   Standard scene duration: ~60-75 frames (2.0-2.5s).
    -   Fast transitions: 6-9 frames (200-300ms).
    -   Text reveal stagger: 4-5 frames between words.
    -   Initial text reveals (e.g., "Turn Books"): ~15 frames total duration.

4.  **Core Remotion patterns**:
    -   `useSpring()` is essential for the snappy UI feel.
    -   `Sequence` components are critical for managing the fast-paced, beat-by-beat structure.
    -   `interpolate()` with `Easing.out()` for most fades and positional tweens.
    -   `remotion/three` would be required to accurately replicate the 3D phone tunnel (00:07) and the spinning cards transition (00:18).
    -   A simple `clip-path: inset(...)` animation can replicate the center-out wipe at 00:05.

5.  **The 2–3 most essential techniques**:
    -   **Rhythm is King**: The single most important element is the tight synchronization of animation to the audio track. Use `useAudioData` to analyze the beat and time keyframes precisely to the music's rhythm.
    -   **Critically Damped Motion**: The "snappy but not bouncy" feel is core to the premium aesthetic. Overly soft or springy animations will miss the mark. Configure springs with high stiffness and damping.
    -   **Motivated Transitions**: Use elements from one scene to drive the transition to the next (e.g., the phone flying into the camera). This creates a sophisticated, continuous flow instead of a simple slideshow.

---

## §3600 — Composition Guidelines: Spatial, Hierarchy, Contrast (SaaS Showcase V1 Audit)

*Added after user audit of `saas-showcase` (Vertex). These three principles were the main source of poor output and are now encoded in `src/data/categories/saas.json` under `compositionGuidelines`.*

### The Problem

The showcase ad had correct animations but violated three fundamental composition rules, making scenes look empty, cluttered, or unreadable:
1. UI cards and stat widgets were sized at 240–320px on a 3840×2160 canvas (~6-8% of width) — they looked like thumbnails on a billboard.
2. Text overlapped visual demonstrations (spring balls animated through subtitle text).
3. White text was placed on a white (#FDFDFE) background — completely invisible.

### Rule 1: Spatial Utilization

> **The focal element of each scene must occupy 30–60% of the canvas width or height.**

- On 4K (3840×2160): UI cards ≥ 400–600px wide, stat widgets at hero scale (40–60% of canvas)
- Dashboard KPIs, counters, and progress rings ARE attention-capture elements — size them as hero elements, not side widgets
- Distribute elements across the full canvas area — use edges and corners, not just the center band
- Leave intentional whitespace, but never leave the center 50% visually empty while content sits in corners

**Before/After (S2 widgets):**
| | Width | Height | % of canvas |
|---|---|---|---|
| Before | 240px | 160px | 6.3% |
| After | 460px | 320px | 12% per widget, ~36% total row |

**Before/After (S3 hero ring):**
| | Radius | Counter font | % of canvas height |
|---|---|---|---|
| Before | 180px | 200px | 16.7% |
| After | 320px | 320px | 29.6% |

### Rule 2: Visual Hierarchy

> **Each scene has one purpose. Identify it first, then design around it.**

- The focal element gets: largest size, center/golden-ratio placement, highest contrast, first spring entrance
- Supporting elements enter AFTER the focal (via stagger delay) and are smaller/more muted
- **Text must NEVER overlap the primary visual** — place text above (top 20%) or below (bottom 20%) the demonstration zone
- If text describes what's happening, it's a subtitle — position it at frame edges, not in the center where it fights the visual

**Before/After (S4 spring physics):**
| | Title position | Ball travel zone | Overlap? |
|---|---|---|---|
| Before | y=CY-340 to CY-200 | CY+200 → CY-200 | Yes — balls animate through subtitle |
| After | y=280 to y=480 (top 22%) | CY+400 → CY-100 (center-low) | No — 400px clear gap |

### Rule 3: Color Contrast

> **Text must always be readable. Minimum 4.5:1 contrast ratio.**

- Never white text on light backgrounds (#FDFDFE, #f2f2f2) — use C.black (#1E1E1E) or accent colors
- Never dark text on dark overlays (#08080e) — switch to white or light accents
- Background blobs/gradients must not wash out overlapping text
- **Thumbnail test**: if text disappears when the composition is viewed at 200px width, it needs more contrast

**Before/After (S5):**
| | Text color | Background | Readable? |
|---|---|---|---|
| Before | #ffffff (white) | #FDFDFE (off-white) | No — invisible |
| After | #1E1E1E (black) + #2076FF (blue) | #FDFDFE (off-white) | Yes — high contrast |

### Where This Lives in Code

- **Category data**: `src/data/categories/saas.json` → `compositionGuidelines` section
- **Schema**: `src/data/categories/schema.ts` → `compositionGuidelines` interface on `Category`
- **Agent rule**: `.cursor/rules/scoped-knowledge.mdc` → Step 4 (checked before technique selection)
- **Showcase comp**: `src/compositions/SaasShowcaseAd.tsx` — all fixes applied and verified

---

## §3700 — Component Animation (SaaS Product Promo AE Extract)

*Added after extracting `SaaS_Product_Promo_Final.ae-extract.json` and building the simple component animation subcategory.*

### Source Material

The SaaS Product Promo Final AE file is a 32.5s (976 frames @ 30fps) 4K product promo for a fictional project management tool called "MotionRam". The main composition routes through a single precomp called "Color" containing 122 layers with clear scene boundaries.

**Key AE reference values:**
- Background: `[0.3709, 0.6947, 1]` → `#5EB1FF` (bright blue, used across all BG solids: BG3–BG11)
- Card fill: `[0.9725, 0.9725, 0.9725]` → `#F8F8F8` (near-white, from `shapeContent.Fill 1.Color`)
- Card stroke: `[0.5148, 0.7355, 1]` → `#83BBFF` (light blue border)
- Accent text: `[0.1059, 0.3647, 1]` → `#1B5DFF` (blue text inside white buttons)
- Card roundness: 603 (pill-shaped buttons), 60 (dashboard cards)
- Font: Inter Medium (500) and SemiBold (600), tracking -60
- Pill button dimensions: 1100×350px (at 4K), borderRadius 603

### Scene Structure from AE

The AE file has these distinct component animation scenes:

| Frames | Content | Animation Pattern |
|--------|---------|-------------------|
| 0–95 | Hook text: "Tired of losing hours switching between tools?" | Text position keyframes (slide in from off-screen), scale 100→34, opacity pop 0→100 in 2 frames |
| 95–189 | "MotionRam Can fix that" + search bar component | Search icon rotates, circle scales, rectangle slides |
| 189–230 | Stat: "-89,129%" "of people waste" + gradient line | Position keyframes with 4 stops, opacity 4 keyframes |
| 329–425 | "MotionRam" hero text + large rounded card (3000×720, r603) | Card has scale+rotation keyframes, stroke version for glow |
| 457–599 | Feature buttons: "Add project", "Assign tasks", "Track progress" | Three pills (1100×350, r603) on blue BG. Parent null position keyframes drive slide. Arrows between buttons. |
| 739–785 | Dashboard UI: 12+ rectangles, stats, testimonial, checkout | Multiple card sizes (r60), tick/cross/circle icons, "95/100", "Great Stuff", "1,200+ users", "Checkout" |
| 839–881 | "Subscribe Now" CTA button + "Organize your work in minutes" | Rectangle 18 with position+opacity keyframes |
| 908–976 | Logo placeholder + tick marks + circles | Scale keyframes on circles and s5 null |

### What "Simple" Means

Simple component animation = basic spring-driven **slide + scale + opacity** with stagger delays. Each component type has a single entrance animation:

- **Pill buttons**: `translateX(600px → 0)` + `scale(0.85 → 1.0)` + `opacity(0 → 1)`, staggered 6 frames apart
- **Stat cards**: `translateY(80px → 0)` + `scale(0.9 → 1.0)`, counter value animates via spring progress
- **Testimonial cards**: `translateX(±200px → 0)` + `scale(0.9 → 1.0)`, stars pop in with icon spring
- **Toggle switches**: Knob slides left→right via spring, track color transitions
- **Notification cards**: `translateX(500px → 0)` from right side
- **CTA buttons**: `scale(0.7 → 1.0)` + post-settle sine pulse (amplitude 0.012–0.015)

### Spring Configs Used

| Element Type | Stiffness | Damping | Mass | Feel |
|-------------|-----------|---------|------|------|
| `pill` | 160 | 24 | 0.85 | Snappy slide-in |
| `card` | 140 | 22 | 0.9 | Slightly heavier for gravitas |
| `icon` | 200 | 20 | 0.7 | Very snappy for checkmarks/stars |
| `cta` | 120 | 18 | 0.9 | Slight bounce for attention |
| `text` | 170 | 25 | 0.8 | Standard text reveal |

### What "Complex" Will Mean (Future)

Complex component animation will go beyond simple entrance animations to include:
- **Inter-component choreography**: Button A completing triggers Card B to slide in → progress fills → badge appears
- **Data-flow visualizations**: Animated connection lines/arrows showing data flowing between components
- **Micro-interactions**: Hover states, click ripples, input field focus rings, cursor animations
- **State transitions**: Component morphing from one state to another (loading → loaded, collapsed → expanded)
- **Nested animations**: Cards that contain sub-components which animate independently
- **Layout reflow**: Components repositioning as new elements enter, grid rearrangements

### File Structure

```
src/compositions/SaasSimpleComponentAd.tsx  — standalone 5-scene ad (18s)
src/compositions/SaasPlaygroundAd.tsx       — S11–S15 embeds SaasSimpleComponentAd
src/data/categories/saas.json               — "component-animation" subcategory
```

### Key Design Decisions

1. **Blue gradient backgrounds** (#5EB1FF → #3A8FE0 → #2E6FC0): Darkening per scene adds variety without changing the palette
2. **White cards on blue**: High contrast, matches the AE extract exactly
3. **Checkmark draw-on**: SVG `strokeDashoffset` animation triggered *after* the button lands (delay = button delay + 20 frames)
4. **One component class per scene**: Keeps each scene focused and demonstrates the technique clearly
5. **Scene-level breathing**: BgCircles with sine-wave float (amplitude 6px, frequency 0.02) add organic life without distracting from components
6. **CTA pulse**: `1 + Math.sin(frame * 0.06) * 0.015 * springProgress` — subtle enough to draw attention without feeling jittery

---

## §3800 — CalmlyAd: Chaos-to-Calm Narrative Ad (Comprehensive Learnings)

*Added after ~20 iteration rounds building `CalmlyAd.tsx` — a 13s (390 frames @ 30fps) 4K SaaS ad for a fictional productivity app "Calmly". This was the most iterated ad to date and produced the richest animation learnings.*

### Overview

3-act structure: **Chaos** (inbox overwhelm) → **The Click** (colorful blob vortex transition) → **Calm** (clean dashboard with drag-and-drop interaction + tagline). Built entirely from scratch using learned AE principles, no template import.

### §3810 — What Worked (Keep Doing)

#### 1. Kinetic Typography Cascade (Act 1)
Each word enters from a **different direction** with a different spring config. This is dramatically more engaging than uniform word-by-word reveals.

| Word | Direction | Spring | Why it works |
|------|-----------|--------|--------------|
| "Your" | Drops from above | `{120, 18, 1.0}` | Gravity feel, calm start |
| "inbox" | Flies out of envelope | `{60, 9, 1.4}` | Bouncy, tied to prop (envelope), distinct blue color + bold weight |
| "never" | Scales up from position | `{140, 16, 0.7}` | Emerges from within, light and snappy |
| "stops." | Slams from below | `{160, 20, 1.0}` | Heaviest impact, screen shake on land, pink color |

**Key insight:** Giving each word a **unique entry vector + spring personality** creates rhythm. The stagger spacing (0, 10, 22, 30 frames) is intentionally uneven — faster acceleration builds tension.

#### 2. Envelope Component (Act 1)
SVG-based envelope with 3D flap rotation that opens to "release" the word "inbox." This **narrative prop** elevates a text reveal from generic to memorable.

**What made it work:**
- SVG renders crisp at any scale (important at 4K)
- Warm paper colors (#E8E4DF body, #F0EFF4 flap) feel tactile
- V-fold panels on the front with differential shading (left #E2DDD6, right #DDD8D1) add depth
- Letter inside rises as flap opens (`letterRise = clamp01((flapOpen - 0.3) / 0.7)`)
- Word timing delays from envelope opening (`enterFrame` offset of 7 frames after flap starts)

#### 3. Screen Shake on Impact (Act 1)
When "stops." slams in, the **other 3 words + notification widgets** shake — but "stops." itself doesn't (it's outside the shake wrapper). This creates the illusion that "stops." **caused** the tremor.

**Parameters:** `sin(st * 4.2) * 6 * exp(-st * 0.5)` for X, `cos(st * 5.7) * 4 * exp(-st * 0.5)` for Y, duration 8 frames. The different frequencies on X vs Y prevent the shake from feeling circular.

#### 4. Epicyclic Motion for Blob Drift (Act 2)
Replaced simple sin/cos oscillations with **dual-rotor epicycles** — two layered circular motions with same-direction rotation. This produces smooth, continuously-varying curves that never reverse or stall.

**Critical parameters:**
- Primary orbit velocity must dominate: `R1 * w1 > 2.5 * R2 * w2`
- 6 distinct variants keyed by blob ID for visual diversity
- Phase alignment: `phi1 = flightAngle + π/2 - driftStart * w1` ensures epicycle velocity matches flight direction at handoff

#### 5. Coast Phase for Momentum Preservation (Act 2)
After blobs fly outward from the burst, they **continue in their flight direction** for 18 frames with decelerating velocity before the drift epicycle takes over. Uses `speed * (t - t²/(2*dur))` for smooth deceleration.

**Why this matters:** Without the coast, blobs would halt abruptly and start drifting sideways — breaking the illusion of physical inertia.

#### 6. Dynamic Camera Focus Tracking (Act 3)
Camera `transformOrigin` follows the drag card through 3 phases: locked on floating card → tracks card during drag → settles on dashboard after drop.

**Better than fixed zoom because:** The audience's eye naturally follows the moving object. When the camera zooms into where the card is (not the dashboard center), it feels like the camera is "finding" the action.

#### 7. Layered Camera Effects (Act 3)
Four distinct effects stacked on one transform:
- **Handheld drift:** 1-2px sine breathing (different frequencies for X/Y/rotation)
- **Drag tilt:** 0.4° roll during drag (sine arc, peaks mid-drag)
- **Drop punch:** 3.5% scale pop on successful drop (decays in 12 frames)
- **Vignette overlay:** Radial gradient darkening edges, opacity tied to zoom level

**Key insight:** Each effect is subtle alone (< 2px, < 0.5°, < 4%) but together they create a "real camera" feel. The `driftEnvelope = zoomInP * (1 - zoomOutP * 0.8)` ties all effects to zoom state so they fade naturally.

#### 8. 4-Beat Tagline Sequence (Act 3)
`"Focus,"` → `"finally"` → underline draws → `"."` drops in. Each beat is a distinct animation type:
- Beat 1: Spring rise + blur reveal (standard)
- Beat 2: Same but heavier weight + accent color (visual emphasis)
- Beat 3: `scaleX` from left, smoothstep easing (geometric, not text)
- Beat 4: Spring fall from above with bounce (punctuation as punchline)

**Why 4 beats > 2 beats:** The underline draw and falling period create **two extra moments of delight** after the words have settled. The period's bounce (damping 10) is the final satisfying "stamp."

#### 9. Narrative Drag-and-Drop Interaction (Act 3)
The cursor picks up a "messy" floating task card, drags it into a clean dashboard, and the dashboard updates (counter increments, checkmark draws on, teal flash). This **tells a product story** through animation instead of just showing a UI.

**Components choreographed:** FloatingTask (chaos) → AnimCursor (agent) → DragCard (transition) → DashboardCard (resolution) → impact squish + recovery bounce.

### §3820 — What Didn't Work (Mistakes & Fixes)

#### 1. Continuous Camera Drift — REVERTED
Added constant subtle camera translation + rotation to Act 3. User called it "unnatural." 

**Lesson:** Continuous drift works for **ambient/background** scenes (camera-wrapper.md) but feels wrong when there's a **focused interaction** happening. Reserve drift for **zoomed-in holds** only, not the full scene.

#### 2. Dashboard Breathing Animation — REVERTED  
Added subtle scale oscillation to the dashboard card. Felt gimmicky on a UI element that should feel solid.

**Lesson:** Organic motion (breathing, pulse) belongs on **abstract elements** (blobs, glows) not on **UI components** that represent real products.

#### 3. Blob Scale Overshoot Pop — REMOVED
Blobs had a `if (localF < flightDuration + 6)` overshoot scale that caused a visible "pop" at frame ~110.

**Lesson:** Scale overshoots on small elements (< 100px) are barely visible and just look like a glitch. Reserve scale bounce for **large elements** (cards, text > 80px) where the overshoot is readable.

#### 4. Sine/Cosine Blob Movement — REPLACED
Initial blob drift used independent sin/cos on X and Y axes. All blobs did similar figure-8 patterns.

**Lesson:** `sin(t) * A` on X and `cos(t) * B` on Y creates **Lissajous figures** which all look similar regardless of frequency. Epicyclic motion (layered same-direction rotations) produces much more varied, naturalistic paths.

#### 5. Quadratic Ease-In for Drift Start — REPLACED WITH LINEAR
Used `t²` ease-in when transitioning from flight to drift. Created visible ~10 frame stall.

**Lesson:** When preserving momentum from a preceding phase, use **linear** ease-in (`t` over 4 frames), not quadratic. Quadratic has zero initial velocity which is exactly what you DON'T want when objects should be in motion.

#### 6. Instant Blob Spawning/Despawning — FIXED
Large background blobs appeared and disappeared in 1-2 frames during the vortex sequence.

**Lesson:** This was the single most frequently reported visual glitch (~5 separate reports). Led to creating the `animation-fluidity.mdc` rule. **Every element needs 8+ frame entrance/exit transitions, no exceptions.**

#### 7. "Pen-Drawn" Wavy Underline — SIMPLIFIED
Initially implemented as an SVG path with quadratic bezier curves (`Q 30 4, 60 9 Q 90 15, 120 8...`). Looked messy rather than elegant.

**Lesson:** For **branded/corporate** aesthetics, clean geometric animations (straight line with `scaleX`) outperform "handmade" effects. Save hand-drawn effects for lifestyle/creative brands.

#### 8. Text Overlap — ITERATIVE FIX
"inbox" and "never" overlapped because positioned too close. "stops." bounced into other text because spring had too much overshoot (stiffness 220, damping 10).

**Lesson:** When words enter from **different directions**, add 30-50% more spacing than you think you need. Each word's spring overshoot temporarily extends its footprint. Test at the **peak overshoot frame**, not the settled frame.

### §3830 — Spring Config Reference Table (CalmlyAd)

*Collected spring configs with their observed behavior. Use as a starting point for new ads.*

| Element | Stiffness | Damping | Mass | Behavior | Notes |
|---------|-----------|---------|------|----------|-------|
| Text drop (standard) | 120–140 | 16–18 | 0.7–1.0 | Clean settle, minimal overshoot | Good for supporting words |
| Text slam (impact) | 160 | 20 | 1.0 | Heavy, decisive, slight bounce | For emphatic final words |
| Text emerge (bouncy) | 60 | 9 | 1.4 | Slow travel, visible bounce | For hero words that need to breathe |
| Envelope body | 140 | 18 | 0.9 | Quick, clean entrance | Matches surrounding elements |
| Envelope flap | 80 | 14 | 1.0 | Slow, deliberate opening | 3D feel needs slower motion |
| Notification widget | 200 | 12 | 0.6 | Snappy pop with bounce | Chaos energy, lightweight |
| Dashboard unfurl | 80 | 18 | 1.2 | Slow, weighty reveal | Heavy mass = gravitas |
| Card pickup | 300 | 18 | 0.5 | Instant snap | Must feel responsive to cursor |
| Drop impact recovery | 280 | 10 | 0.5 | Fast, bouncy | Satisfying "thunk" |
| Camera zoom in | 60 | 12 | 1.0 | Gradual with overshoot | Feels like real dolly move |
| Camera zoom out | 55 | 14 | 1.2 | Slightly heavier pull-back | Cinematic, unhurried |
| Camera drop punch | 220 | 18 | 0.6 | Brief impulse, quick decay | ~3.5% scale, 12 frame window |
| Period drop | 160 | 10 | 0.8 | Bouncy fall | Punctuation as punchline |
| Tagline "Focus," | 140 | 16 | 0.8 | Standard reveal | Restrained, warm |
| Tagline "finally" | 120 | 13 | 0.9 | Slightly bouncier | Heavier word deserves more motion |
| Post-drop focus settle | 30 | 16 | 1.5 | Very slow, heavy drift | Camera settling, not snapping |

### §3840 — Timing & Pacing Insights

**Act duration ratios (CalmlyAd final):**
- Act 1 (Chaos): 90 frames / 3.0s — **23%** of total
- Act 2 (Transition): 130 frames / 4.3s — **33%** of total  
- Act 3 (Calm + CTA): 170 frames / 5.7s — **44%** of total

**Key timing lessons:**
1. **Act 2 was originally 2.3s — doubled to 4.3s.** The vortex transition needed time to breathe. Complex transitions need **at least 4 seconds** to read properly.
2. **Tagline needs 1+ second of hold after settling.** At 30fps, the last word should enter no later than frame `total - 16` (about 0.5s from end) for the audience to read it.
3. **Notification widgets stagger at 4-frame intervals.** Tighter than standard 15-25 frame stagger because they're background/ambient elements, not focal.
4. **Screen shake must be short (8 frames max).** Longer shakes become annoying rather than impactful. Decay via `exp(-t * 0.5)` ensures 90% attenuation by frame 5.
5. **Camera effects should be tied to content, not time.** Pan strength `= zoomInP * (1 - zoomOutP)` auto-scales all effects to zoom state. Never hardcode camera effect timing separately from what's being filmed.

### §3850 — Reusable Patterns Identified

These patterns from CalmlyAd are general-purpose and should be extracted for future ads:

1. **Kinetic Typography Cascade** — per-word entry from different directions with unique springs
2. **SVG Envelope Component** — narrative prop for email/inbox concepts
3. **Epicyclic Drift** — smooth, varied organic motion for floating elements
4. **Dynamic Camera Focus** — transformOrigin tracking a moving element
5. **Layered Camera Stack** — drift + tilt + punch + vignette on one transform
6. **4-Beat Tagline** — word → word → geometric accent → punctuation drop
7. **Narrative Drag Interaction** — cursor + floating card → dashboard integration
...
10. **Vortex Convergence** — spiral motion with radius decay + opacity fade for scene transitions

---

## §3900 — Hook Scenes vs Showcase Scenes: Different Rules

*Added after GreenTask S1 audit — the "what can be removed?" principle was incorrectly applied to a narrative hook.*

### The Mistake

LEARNINGS §1409 says *"Premium ads let the frame stay sparse so the focal element feels important"* and *"What can be removed from this frame?"* This is correct for **product showcase scenes** (S2–S5) where UI elements, stats, and features need clean visual hierarchy. It is **wrong** for hook scenes.

### Why Hooks Play by Different Rules

Research (2025-2026 ad performance data) shows:

1. **Kinetic energy captures attention.** "Kinetic energy" (perceived movement intensity) is a measurable metric. High kinetic energy hooks stop the scroll. The goal is "purposeful energy, not noise" — many moving parts are fine if they're choreographed toward a single narrative.

2. **Visual richness signals quality.** Within 50ms, viewers judge whether content looks professional. Detailed, polished animation triggers the brain's reward system and signals premium brand positioning. Sparse hooks can read as "cheap" or "boring."

3. **Motion reduces cognitive load, it doesn't increase it.** Motion graphics act as "cognitive support" — they guide attention moment by moment and maintain visual novelty. A vine growing with leaves popping and particles rising creates a living system the eye naturally follows. This is easier to process than static complexity.

4. **The real danger is chaos, not complexity.** The distinction is between orchestrated complexity (many elements serving one metaphor, one direction, one narrative) and visual noise (many unrelated elements competing). A vine with leaves, branches, tendrils, and text growing from a single checkbox upward is orchestrated. Random floating icons would be noise.

### The Rule

| Scene type | Visual density | Guiding question |
|---|---|---|
| Hook (S1) | Rich, kinetic, many moving parts OK | "Does everything serve the same narrative?" |
| Product showcase (S2–S5) | Clean, sparse, focal clarity | "What can be removed?" |
| CTA (final) | Medium — payoff energy with clear hierarchy | "Is the action unmissable?" |

### What to Audit in a Hook

Instead of counting elements and saying "too many," audit:
- Do all elements share a single origin point or narrative thread?
- Is there a clear direction of motion (upward, outward, converging)?
- Does the choreography create a readable sequence, or do things fire simultaneously?
- Does the visual richness serve the hook's message, or is it decoration unrelated to the concept?

If the answer to all four is yes, the complexity is an asset, not a problem

---

## §3950 — SVG Proportioning Rules (Mandatory for All Novel Components)

*Added after GreenTask S1 vine rework — leaves were 52px in a 1600px container (3.25%), rendering them as invisible specks at 4K. The root cause was "scale blindness": setting pixel values without calculating their proportion to the canvas.*

### The Rule: Always Work Backwards from Proportions

Before setting ANY pixel dimension for a novel SVG/CSS element:

```
1. Determine the container size (the area this element lives in)
2. Determine the desired visual proportion (from reference or design intent)
3. Calculate: target_size = container_size × proportion
4. Use the calculated value, not a "feels right" number
```

### Proportion Reference Table

| Visual role | Target % of container | Example (1600px container) |
|---|---|---|
| Hero/dominant element | 30-50% | 480-800px |
| Prominent detail (leaf, icon, card) | 10-15% | 160-240px |
| Supporting detail (badge, label) | 5-8% | 80-128px |
| Ambient/particle | 1-3% | 16-48px |
| Invisible threshold (DON'T go below) | < 1% | < 16px |

### SVG Stroke Width Hierarchy

Multiple SVG stroke levels must follow a consistent taper:

| Level | % of container width | Ratio | Example (1600px) |
|---|---|---|---|
| Primary (stem/trunk) | 1.5% | 1.0x | 24px |
| Secondary (branch) | 0.8-0.9% | 0.5-0.6x | 12-14px |
| Tertiary (tendril) | 0.3-0.4% | 0.2-0.3x | 5-6px |
| Detail (vein, hair) | 0.1-0.2% | 0.08-0.1x | 2-3px |

### Leaf/Detail Clustering Rules

Individual elements that represent organic clusters (leaves, particles, cards):
- Cluster 3-4 elements per attachment point, overlapping by 20-40%
- Include 1 "ghost" element per cluster (low opacity, offset behind) for depth
- Vary shape across 3-6 variants — never all identical
- Largest element in each cluster should be 1.2-1.5x the smallest

### The Checklist (Run Before Every Novel Component)

- [ ] Element width / container width = what %? Is it > 1%?
- [ ] Does this match the intended visual role (hero, detail, ambient)?
- [ ] Stroke widths follow the hierarchy (primary > secondary > tertiary)?
- [ ] Clusters have 3+ elements with overlap, not isolated individual items?
- [ ] At least one ghost/depth layer behind each cluster?
- [ ] Shape variety: 3+ shape variants for repeated elements?

### Where This Lives in the Pipeline

The **Art Director** (Step 3) is responsible for calculating these proportions
and documenting them in `docs/ads/{name}/art-direction.md` before the
Prompt Director writes any code. See `.cursor/skills/art-director/SKILL.md`.

## §4000 — SVG Asset Creation: AI Reference → Hand-Craft (Never AI-to-SVG)

*Added after GreenTask S1 plant asset — tested QuiverAI Arrow for both image-to-SVG and text-to-SVG, both produced unusable output.*

### The Mistake
We built an entire `scripts/generate-asset.js` pipeline around QuiverAI Arrow to convert reference images or text prompts into production SVGs. Both approaches failed:

1. **Image-to-SVG** (`vectorizeSVG`): Traced the reference literally — including background, text labels as vector paths, and faint gradient fills. The plant stem and leaves were nearly invisible, the structure was a single merged blob, and text from the reference became vector garbage.
2. **Text-to-SVG** (`createSVGs.generateSVG`): Produced one massive merged path containing stem, branches, and leaves all fused together. No logical grouping, no checkbox rectangle, no animation-ready structure.

Both outputs were fundamentally unusable because AI SVG generators optimize for *visual similarity* not *structural quality*. They produce the fewest paths needed to approximate the image, merging everything.

### The Proven Workflow: AI Reference Image → Hand-Craft SVG

| Step | Tool | Output |
|------|------|--------|
| 1. Generate reference | `GenerateImage` tool or user-provided image | High-quality 2D illustration showing target visual |
| 2. Study reference | Human/AI analysis | Identify shapes, proportions, colors, detail patterns |
| 3. Hand-craft SVG | Manual path writing | Clean SVG with `<g>` groups, `id` attributes, logical structure |

### Hand-Craft SVG Checklist
- [ ] Each animatable part is a separate `<g id="...">` group
- [ ] Leaf/detail elements have vein lines (midrib + 3 pairs of side veins as a single multi-move path)
- [ ] Ghost/depth copies behind main elements at 8-12% opacity with visible structural detail
- [ ] Small bud/accent elements at junction points (bright green, tiny, with just a midrib)
- [ ] All paths use inlined coordinates (avoid `<defs>`/`<use>` — some renderers break)
- [ ] Solid fills only, no gradients (cleaner, faster, easier to animate)
- [ ] viewBox sized appropriately (e.g., `0 0 400 650` for a vertical plant)
- [ ] Explicit `width` and `height` attributes on root `<svg>` element

### Leaf Vein Pattern (Reusable)
For a leaf shape `M 0 42 C -20 28 -23 -12 0 -42 C 23 -12 20 28 0 42 Z` (46w × 84h):
```
Midrib + 3 side vein pairs as a single path:
M 0 38 L 0 -38 M 0 22 L -16 12 M 0 22 L 16 12 M 0 6 L -18 -4 M 0 6 L 18 -4 M 0 -10 L -14 -20 M 0 -10 L 14 -20

Vein colors:
- Dark green leaf (#065F46): veins stroke="#033D2E" opacity="0.3"
- Medium green leaf (#22C55E): veins stroke="#065F46" opacity="0.25"
- Ghost leaf: veins stroke="#065F46" opacity="0.25" (most visible — the "X-ray" effect)
```

### Why This Matters
AI image generators are excellent at *visual design* (composition, style, detail). AI SVG generators are terrible at *structural design* (grouping, layering, animation-readiness). Using each tool for what it's good at produces dramatically better results than trying to make one tool do everything.

## §5000 — Sighted SVG Pipeline: Semantic Color Maps + Render-Read-Refine

*Added after GreenTask S1 plant asset — replaced both hand-crafted SVG and blind-tracing workflows with an AI-powered pipeline that solves the "blindness" problem.*

### The Problem
The AI agent cannot see what it produces when writing SVG code. Hand-crafting SVGs from reference images is slow and error-prone. Simple tracing (vtracer) produces accurate shapes but no semantic structure for animation — everything is a flat list of paths grouped only by color proximity or spatial position.

### The Solution: Two Complementary Approaches

#### Approach 1: Semantic Color Map Generation
Instead of generating a normal reference image and trying to separate elements after tracing, generate the reference image with **distinct semantic colors per element type**. After tracing, colors naturally become animation groups.

**Semantic Color Palette Convention:**
| Color | Hex | Semantic Role |
|-------|-----|---------------|
| Red | `#FF0000` | Primary structure (trunk, stem) |
| Blue | `#0000FF` | Secondary structure (branches) |
| Green | `#00FF00` | Primary detail (large leaves) |
| Yellow | `#FFFF00` | Secondary detail (buds, sprouts) |
| Magenta | `#FF00FF` | Accents (flowers, decorations) |
| Cyan | `#00FFFF` | Base/container |
| Orange | `#FF8000` | Tertiary elements |
| White | `#FFFFFF` | Background (discarded via `_background` in color map) |

**Full Pipeline (single command):**
```
node scripts/sighted-pipeline.js <reference-image> <output-dir> --color-map-file <map.json> --recolor-file <recolor.json>
```
This runs: trace → semantic optimize → recolor → split → render preview — all in one shot.
**CRITICAL:** Execute this immediately after GenerateImage. Do NOT pause to plan between image generation and pipeline execution.

**Key scripts:**
- `scripts/sighted-pipeline.js` — **End-to-end pipeline (USE THIS)**
- `scripts/trace-image.js` — Trace raster → SVG via vtracer
- `scripts/optimize-svg.js --mode semantic` — Group paths by semantic color
- `scripts/recolor-svg.js --map-file` — Remap colors to brand palette
- `scripts/split-svg-groups.js` — Split grouped SVG into per-layer files
- `scripts/render-svg.js` — Render SVG to PNG for visual feedback

#### Approach 2: Render-Read-Refine Loop
The agent CAN see images in Cursor via the Read tool (supports PNG, JPG, WebP). By rendering SVGs to PNG files on disk, the agent can read them back and visually verify the output against the reference image.

**When to use visual checkpoints:**
- After tracing: verify the trace captured the right shapes
- After optimization: verify important details weren't lost
- After recoloring: verify the brand palette looks right
- After splitting: verify individual layers are correct

**Context window consideration:** Each image read consumes tokens. Limit visual checkpoints to 4-6 per conversation. Use text-based metrics (file size, path count) for intermediate checks.

### Results (GreenTask Plant)
| Metric | Before (hand-craft) | After (semantic pipeline) |
|--------|--------------------|-----------------------------|
| Asset quality | Low — blind coordinate placement | High — AI-generated shapes with vtracer fidelity |
| Semantic groups | Manual, error-prone | Automatic from color separation |
| Animation control | Per-band clip-path hack | Per-semantic-group independent springs |
| Time to produce | ~30 minutes per asset | ~5 minutes per asset |
| Pipeline automation | None | Fully scripted, repeatable |

### Critical Lessons
1. **AI generates quality, color-coding generates structure.** Don't try to make the AI understand animation groups — encode the grouping in the generation prompt through distinct colors.
2. **Background handling is essential.** Always include `"#FFFFFF": "_background"` in the color map. Groups starting with `_` are automatically discarded by optimize-svg.js.
3. **PowerShell and JSON don't mix.** Always use `--color-map-file` or `--map-file` flags with JSON files instead of inline JSON on PowerShell. Use `-Encoding ascii` when writing JSON files from PowerShell to avoid BOM issues.
4. **Split, don't inline.** For Remotion animation, each semantic group should be its own SVG file so it can be independently positioned, transformed, and animated as a separate `<img>` element with its own spring physics.
5. **Never pause to "plan moves" after GenerateImage.** The pipeline is deterministic — run `sighted-pipeline.js` immediately. All 5 substeps are chained into one command. Deliberating between image generation and pipeline execution wastes time and stalls the workflow.
